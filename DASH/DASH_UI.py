...
if __name__ == "__main__":
    ...
    def get_parts_product_data(reload_data, save_excel):
        today_str, last_day_prev_month_str = get_date_info()
        df_parts = Unleashed_parts_product_forecast_data(start_date='2022-01-01', end_date=last_day_prev_month_str,
                                                         reload=reload_data, save_excel=save_excel)
        df_parts = df_parts.loc[df_parts['Year-Month'] <= last_day_prev_month_str]
        return df_parts
    ...
    def get_other_parts_product_data(top_parts, reload_data, save_excel):
        today_str, last_day_prev_month_str = get_date_info()
        df_parts = Unleashed_parts_other_product_forecast_data(top_parts=top_parts, start_date='2022-01-01',
                                                               end_date=last_day_prev_month_str, reload=reload_data,
                                                               save_excel=save_excel)
        df_parts = df_parts.loc[df_parts['Year-Month'] <= last_day_prev_month_str]
        return df_parts
    ...
    def get_accessories_product_data(reload_data, save_excel):
        today_str, last_day_prev_month_str = get_date_info()
        df_accessories = Unleashed_accessories_product_forecast_data(start_date='2022-01-01',
                                                                     end_date=last_day_prev_month_str, reload=reload_data,
                                                                     save_excel=save_excel)
        df_accessories = df_accessories.loc[df_accessories['Year-Month'] <= last_day_prev_month_str]
    ...
    def get_other_accessories_product_data(top_accessories, reload_data, save_excel):
        today_str, last_day_prev_month_str = get_date_info()
        df_accessories = Unleashed_accessories_other_product_forecast_data(top_accessories=top_accessories,
                                                                           start_date='2022-01-01',
                                                                           end_date=last_day_prev_month_str,
                                                                           reload=reload_data, save_excel=save_excel)
        df_accessories = df_accessories.loc[df_accessories['Year-Month'] <= last_day_prev_month_str]
            ...
            if product_category == 'b':  # go into bikes
                ...
                if parts_input == 's':
                    ...
                    if reload_dash_app == "y":
                        ...
                        dash_parts_launch(series=product_series, financial_forecast=poll_forecast, product_name=product_name,
                                          value_string=value_string, retrain=retrain, path=path,
                                          forecast_horizon=forecast_horizon)
                    ...
                    if reload_dash_app == "y":  # go into dash app
                        ...
                        dash_parts_other_launch(series=product_series, financial_forecast=poll_forecast,
                                                product_name=product_name, value_string=value_string, retrain=retrain,
                                                path=path, forecast_horizon=forecast_horizon, top_parts=parts_to_remove_list)
                ...
                if accessories_input == 's':
                    ...
                    if reload_dash_app == "y":
                        ...
                        product_series = df_accessories.loc[df_accessories['ProductDescription'] == product_name].reset_index(
                            drop=True)
                        dash_accessories_launch(series=product_series, financial_forecast=poll_forecast,
                                                product_name=product_name, value_string=value_string, retrain=retrain,
                                                path=path, forecast_horizon=forecast_horizon)
                    ...
                    df_accessories_other = get_other_accessories_product_data(accessories_to_remove_list, reload_data=True,
                                                                              save_excel=True)
                    ...
                    if reload_dash_app == "y":  # go into dash app
                        ...
                        dash_accessories_other_launch(series=product_series, financial_forecast=poll_forecast,
                                                      product_name=product_name, value_string=value_string, retrain=retrain,
                                                      path=path, forecast_horizon=forecast_horizon,
                                                      top_accessories=accessories_to_remove_list)
